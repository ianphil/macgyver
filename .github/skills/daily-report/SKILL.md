...
**Query 2a — Teams: Monitored chat rollups (direct read)**

**Invoke `skill(teams)` before running any Teams queries.** Read recent messages from monitored chats using the MCPorter CLI.

For each chat listed in `## Teams Channels` in global copilot instructions:

1. Run: `npx mcporter call teams ListChatMessages chatId:"<thread_id>" top:50`
2. Scan the returned messages for items since yesterday
3. Summarize: decisions, action items, open questions, mentions of you or your work items, knowledge drops

Run all chat reads in parallel. Organize findings per-chat in the report.

> **Fallback:** If MCPorter fails, fall back to a WorkIQ query:
> ```
> workiq ask -q "Summarize messages sent since {YESTERDAY_DATE} in the Teams chat thread {THREAD_ID} ('{CHAT_NAME}'). Organize by: mentions of me, open questions, decisions, action items, knowledge sharing. Be concise. Bullet points only. No follow-up offers."
> ```
...
List initiative folder names from `C:\src\miss-moneypenny\initiatives` — names only, do not dive into contents.
...
### Step 4: Deliver to Teams

After assembling the report, post it to Ian's personal Teams Notes chat so it's waiting in Teams each morning.

**Invoke `skill(teams)` if not already loaded.** Then post using MCPorter:

```powershell
npx mcporter call teams PostMessage chatId:"48:notes" content:"<report_content>" contentType:"html"
```

- Use **HTML formatting** with `contentType:"html"` — `<h3>`, `<b>`, `<ul>`, `<table>`, `<code>`, `<hr>` all render correctly. See `expertise/teams-message-formatting.md` for the full reference.
- Send raw HTML tags, NOT entity-encoded (`<b>` not `&lt;b&gt;`)
- End every post with `-- Moneypenny (Ian's AI CoS)`
- Post the full report as a single message
- If the Teams post fails, note the failure but still display the report in the CLI output — delivery is best-effort, not blocking
